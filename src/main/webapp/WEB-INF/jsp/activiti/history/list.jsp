...
function del(){
	...
	if(row.length!=0) {
		window.location='${pageContext.request.contextPath}/activiti/history/toViewImage.do?taskId='+row[0].id;
	}else{
...
function del2(){
	var row = $("#dg").datagrid('getSelections');
	if(row.length!=0) {
		window.location='${pageContext.request.contextPath}/activiti/history/toUpdate.do?id='+row[0].id;
	}else{
		$.messager.alert('提示','请选择一条');
	}
}
