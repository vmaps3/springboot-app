	...
	if(row.length!=0) {
		 $.messager.confirm("确认", "确认删除吗？", function (r) {
		        if (r) {
		        	...
		        	$.ajax({   
					     url:'${pageContext.request.contextPath}/system/role/delete.do',   
					     type:'post',   
					     data:"id="+myArray,
...
function upd(){
	var row =  $("#dg").datagrid('getSelections');
	if(row.length==1){
		window.location="${pageContext.request.contextPath}/system/role/toUpdate.do?id="+row[0].id;
	}else{
		$.messager.alert('提示','请选择一条');
	}
}
...
<body>
   <table id="dg"  class="easyui-datagrid"  fit="true" 
            url="${pageContext.request.contextPath}/system/role/list.do"
    ...
    <div id="toolbar">
    	...
    	<shiro:hasPermission name="/system/role/add">
        	<a href="${pageContext.request.contextPath}/system/role/toAdd.do" class="easyui-linkbutton" iconCls="icon-add" plain="true" >新增</a>
        </shiro:hasPermission>
